...
from model_tt_dark_pt import make_model, load_data as load_data_tt
...
inputs = model_b.input_titles + [["met_pt", "met_phi", "met_x", "met_y"] + feat("lep") + feat("alep") + ["MT2ll"]]
targets = [
...
model = make_model([t.shape for t in train[0]], inputs, [model_bbar], [0, 2], 4, 1000, "relu", 0.25, targets, 0.001, 0)
norm = load_normalization(args.traindata)
...
targets = feat("top") + feat("atop") +\
          ...
          featc("sonnentop") + featc("sonnenatop") + feat("ttbar") + ["mtt", "chel"] + ["gendmmed_x", "gendmmed_y"]
